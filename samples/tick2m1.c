...
{
    if (mm < c_dstp->mm1 || mm > c_dstp->mm2)
        return (FALSE);
    if (mm == c_dstp->mm1 && dd < c_dstp->dd1)
        return (FALSE);
    if (mm == c_dstp->mm2 && dd > c_dstp->dd2)
        return (FALSE);
    return (TRUE);
}
...
void    write1bar(FILE *fpout, time_t bartime, int digits, double op, double hi,
                double lo, double cl, int vol, int spread)
{
    struct tm *tmp;
    tmp=localtime(&bartime);
    fprintf(fpout, "%04d%02d%02d,%02d:%02d,%7.*f,%7.*f,%7.*f,%7.*f,%d,%d\n",
            tmp->tm_year+1900, tmp->tm_mon+1, tmp->tm_mday, tmp->tm_hour, tmp->tm_min,
            digits, op, digits, hi, digits, lo, digits, cl, vol, spread);
}
int      main(int argc, char *argv[])
{
    ...
    if (digits < 0 || digits > 5)
        digits = 0;
#endif
    if (i_year >= 2009 && i_year < 2016)
        c_dstp = dst_range + (i_year-2009);
    else
    {
