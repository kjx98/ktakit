...
int      main(int argc, char *argv[])
{
    ...
    if (argc > 2)
        barstart = atoi(argv[2]);
    else
        barstart = 0;
    if (argc > 3)
        barcount = atoi(argv[3]);
    else
        barcount = 100;

#ifdef  OMMIT
    printf("Size short: %d, Size int: %d, Size long: %d, Size float: %d, Size double: %d\n",
            (int)sizeof(short), (int)sizeof(int), (int)sizeof(long),(int)sizeof(float),(int)sizeof(double));
...
// long 4/4, float 4/4 double 4/8
    ...
    printf("Comm Type %s per %s base %f\n", strp,
            (fhdr.i_comm_lots!=0)?"Deal":"Lot", fhdr.d_comm_base);
    printf("i_start_period: %d, %d, %d, ...\n", fhdr.i_start_period[0],
            fhdr.i_start_period[1], fhdr.i_start_period[2]);
    dt = (time_t)fhdr.t_fromdate;
    tmp = gmtime(&dt);
    printf("Tick Begin date : %04d-%02d-%02d %02d:%02d:%02d,  ", tmp->tm_year+1900,
    ...
    tmp = gmtime(&dt);
    printf("Tick i_from time : %04d-%02d-%02d %02d:%02d:%02d,  ", tmp->tm_year+1900,
            tmp->tm_mon+1, tmp->tm_mday, tmp->tm_hour, tmp->tm_min, tmp->tm_sec);
    dt = (time_t)fhdr.i_to;
    tmp = gmtime(&dt);
    ...
    while (!feof(fp))
    {
        ...
        if (tmp == NULL)
            break;
        printf("%04d%02d%02d %02d:%02d:%02d ", tmp->tm_year+1900, tmp->tm_mon+1,
                tmp->tm_mday, tmp->tm_hour, tmp->tm_min, tmp->tm_sec);
        printf("%7.5lf %7.5lf %7.5lf %7.5lf %ld %d, %d", rates.open,
                rates.high, rates.low, rates.close, (long)rates.tick_volume,
                rates.spread, rates.fill0);
