...
void     tradingsystem(taBars b1, taArray ma1, taArray ma2, taArray trades)
{
    ...
    while (taProcessOrders(&taOS))
    {
        ...
        if (taOS.Current.position == taLONG
        && taArrayItem(b1.lo, taOS.Current.barnum) < taArrayItem(ma2, taOS.Current.barnum))
            taSellToClose(&taOS, "long exit", "", taOS.Current.totalvolume,
                            taArrayItem(b1.lo, taOS.Current.barnum), taSTOP, taDAY);

        /* Evaluate exit rule for short trades */
        if (taOS.Current.position == taSHORT
        && taArrayItem(b1.hi, taOS.Current.barnum) > taArrayItem(ma2, taOS.Current.barnum))
            taBuyToClose(&taOS, "short exit", "", taOS.Current.totalvolume,
                            taArrayItem(b1.hi, taOS.Current.barnum), taSTOP, taDAY);
    }
