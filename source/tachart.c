...
int DLL_EXPORT   taChartSetup(struct taChartScreen *screen, struct taChartInfo *info)
{
    ...
    for (i1 = 0; i1 < screen->linecnt; i1++)
    {
        ...
        else if (info->subystart[screen->chartline[i1].chartnum] == 0 &&
                info->subystop[screen->chartline[i1].chartnum] == 0 &&
                 screen->chartline[i1].ystart != screen->chartline[i1].ystop)
        {
            info->subystart[screen->chartline[i1].chartnum] = screen->chartline[i1].ystart;
            info->subystop[screen->chartline[i1].chartnum] = screen->chartline[i1].ystop;
...
int  DLL_EXPORT  taChartPaint(struct taChartScreen *screen, struct taChartInfo *info)
{
    ...
    if (screen->datestyle && !taIsNoBars(info->masterbars) &&
        (taArrayItem(info->masterbars.dt, screen->xstop) - taArrayItem(info->masterbars.dt, screen->xstart)) >= 1)
    {
        ...
        for (i1 = 0; i1 < screen->xwidth && screen->xstart + i1 < screen->xstop; i1++)
        {
            ...
            if (buffer[0])
            {
                ...
                setcolor((int)screen->bgdcolor);
                rectangle(i3 + 1, info->bottom + 2, i3 + 1 + strlen(buffer) * info->fontwidth,
                            info->bottom + 1 + info->fontheight);
        ...
        if (screen->subchartsize[i1])
        {
            ...
            if (!i3)
            {
                f2 = taChartRoundMinMax(info->subtop[i1], info->subtop[i1 + 1] - 2,
                        info->subystart[i1], info->subystop[i1], NULL, NULL, info->fontheight);
                if (f2)
                {
                    for (f1 = info->subystart[i1] + f2; f1 <= info->subystop[i1] - f2; f1 += f2)
                    {
                        if (screen->hgrid != taNULLSTYLE)
                            taChartGrid(f1, info->subystart[i1], info->subystop[i1],
                                    info->left, info->subtop[i1], info->right, info->subtop[i1 + 1] - 2,
                                    screen->gridcolor, screen->hgrid);
                        if (screen->scalepos)
                        {
                            setcolor((int)screen->numcolor);
                            i2 = taChartCoord(f1, info->subystart[i1], info->subystop[i1],
                                            info->subtop[i1] + 1, info->subtop[i1 + 1] - 2 - 1);
    ...
    if (screen->scalepos)
    {
        ...
        {
            setcolor((int)screen->numcolor);
            i2 = taChartCoord(screen->chartgrid[i1].value, screen->chartgrid[i1].ystart,
                            screen->chartgrid[i1].ystop, info->subtop[screen->chartgrid[i1].chartnum],
                            info->subtop[screen->chartgrid[i1].chartnum + 1] - 2);
...
KFloat    taChartRoundMinMax(int y1, int y2, KFloat ymin, KFloat ymax, KFloat *ystart,
                            KFloat *ystop, int fontheight)
{
    int    i1;
    KFloat    f1, factor = 1;
                                 ...
                                 int x1, int y1, int *barnum, int *barpos, int *subchart, KFloat *yvalue)
{
    ...
    *yvalue = 0;
    for (i1 = 0; i1 < taCHART_SUBCNTMAX; i1++)
    {
        ...
        {
            *subchart = i1;
            *yvalue = info->subystart[*subchart] + (info->subtop[*subchart + 1] - y1)
                        * (info->subystop[*subchart] - info->subystart[*subchart])
                        / (info->subtop[*subchart + 1] - info->subtop[*subchart]);
            break;
        }
    }
    return (1);
