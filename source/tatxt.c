                         ...
                         long start, int cnt, int allocate)
{
    ...
    for (i1 = 0; i1 < info->skip; i1++) fgets(buffer, 255, fp1);
    do
    {
        if (l1 == start)
            fpos = ftell(fp1);
    ...
    if (cnt < 0)
    {
        if (start > 0) cnt = (int)(min(reccnt, 16000) - start);
        else cnt = (int) min(reccnt, 16000);
    }
    if (allocate)
    {
    ...
    while (i1 < cnt && fgets(buffer, 255, fp1) != NULL)
    {
        ...
        {
            if (i2 == 1) string = strtok(buffer, info->delimit);
            else string = strtok(NULL, info->delimit);
            if (info->dt == i2)
                taArrayItem(b1->dt, i1) = taDateTxtToJulian(string, info->dtformat);
            if (info->ti == i2)
                taArrayItem(b1->ti, i1) = taTimeTxtToJulian(string, info->tiformat);
            if (info->op == i2) taArrayItem(b1->op, i1) = atof(string);
            if (info->hi == i2) taArrayItem(b1->hi, i1) = atof(string);
            if (info->lo == i2) taArrayItem(b1->lo, i1) = atof(string);
            if (info->cl == i2) taArrayItem(b1->cl, i1) = atof(string);
            if (info->vol == i2) taArrayItem(b1->vol, i1) = atof(string);
            if (info->oi == i2) taArrayItem(b1->oi, i1) = atof(string);
        }
        i1++;
    }
    fclose(fp1);
...
int DLL_EXPORT taTXTwriteinfo(char *path, char *outfile, struct taTXTinfo *info)
{
    ...
    if ((fp1 = fopen(fullname, "wt")) == NULL) return (taErrFileOpen);
    for (i1 = 0; info->delimit[i1]; i1++)
        if (info->delimit[i1] == ' ') info->delimit[i1] = '_';
