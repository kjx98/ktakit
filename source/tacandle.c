...
KFloat  acr = 0; /* AverageCandleRange */
    ...
    for (pos = 0; pos < b1->size; pos++)
    {
        ...
        if (len > acr) info->candle[pos].shape = taLARGE;
        else if (len > realbodybot * info->equalpct)
            info->candle[pos].shape = taSMALL;
        else info->candle[pos].shape = taDOJI;
        if (info->candle[pos].color == taNONE) len = info->equalpct * realbodytop;
        ...
        else info->candle[pos].topshadow = taMEDIUM;
        ...
        else info->candle[pos].botshadow = taMEDIUM;
    }
    return (0);
}
