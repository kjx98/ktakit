...
unsigned int taCSIdatetojulian(float date)
{
 ...
 return (taYMDToJulian((int) year + 1900, (int) month, (int) day));
}
...
long DLL_EXPORT taCSIfindrec(char *path, char *name, unsigned int date, unsigned int *actualdate)
{
 ...
 if ((fnum = taCSIfindfile(path, name, &mrec)) < 0)
  return (fnum);
 if (path[0] != '\0' && path[strlen(path) - 1] != ':' && path[strlen(path) - 1] != '\\')
 ...
 if (dt > hdr.lpdof)
 {
  fclose(fp1);
  if (actualdate)
   *actualdate = taCSIdatetojulian(hdr.lpdof);
 ...
 fclose(fp1);
 if (actualdate)
  *actualdate = taCSIdatetojulian(rec.date);
 if (err)
  return (err);
 return (recno);
}
...
int DLL_EXPORT taCSIread(taBars *b1, char *path, char *name, long start, int cnt, int allocate)
{
 ...
 {
  if (start > 0)
   cnt = min((short ) hdr.mdp, 16000) - start;
  else
   cnt = min((short ) hdr.mdp, 16000);
 }
 ...
 if (mrec.oflag == 'P')
  ...
  b1->optiontype = taCALL;
 else
  b1->optiontype = taUNKNOWN;
 #ifdef    OMMIT
