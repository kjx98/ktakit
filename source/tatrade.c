...
int DLL_EXPORT taProcessOrders(struct taOrderSystem *taOS)
{
 ...
 for (i1 = 0; i1 < taOS->OrderCnt; i1++)
 {
  ...
  if ((unsigned)taOS->Order[i1].time + (unsigned)taOS->OrderParms.ordertimedelay > (unsigned)taArrayItem(taOS->bars.ti, taOS->Current.barnum))
   continue;
...
double   taCalcProfit(struct taOrderSystem *taOS, long in, struct taExecution OutExec)
{
 ...
 if (taOS->Net.loss)
  taOS->Net.avgloss = taOS->Net.grossloss / taOS->Net.loss;
 if (taOS->Net.avgloss)
...
int taSortOrders(const void *elem1v, const void *elem2v)
{
 ...
 if (elem1->type < elem2->type)
  return (1);
 if (elem1->buysell < elem2->buysell)
  return (-1);
 if (elem1->buysell > elem2->buysell)
  return (1);
 if (elem1->reqprice == elem2->reqprice)
  return (0);
