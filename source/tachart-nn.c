...
int    taChart(taArray *pa1, taBars *pb1, int xstart, int xstop, int xwidth,
           ...
           int type, long color1, long color2, long style)
{
 ...
 case taCHART_OPHILOCL:
  for (pos = xstart; pos <= xstop && pos < b1.size; pos++)
  {
   ...
   moveto(x3, y2 - (short) ((taArrayItem(b1.hi, pos) - ystart) * yscale));
   lineto(x3, y2 - (short) ((taArrayItem(b1.lo, pos) - ystart) * yscale));
   moveto(x3 + 1, y2 - (short) ((taArrayItem(b1.cl, pos) - ystart) * yscale));
   lineto(x3 + halfxscale, y2 - (short) ((taArrayItem(b1.cl, pos) - ystart) * yscale));
   if (type == taCHART_OPHILOCL && taArrayItem(b1.op, pos))
   {
    moveto(x3 - 1, y2 - (short) ((taArrayItem(b1.op, pos) - ystart) * yscale));
    lineto(x3 - halfxscale, y2 - (short) ((taArrayItem(b1.op, pos) - ystart) * yscale));
   }
  }
  break;
 ...
 case taCHART_CANDLE:
  ...
  for (pos = xstart; pos <= xstop && pos < b1.size; pos++)
  {
   ...
   moveto(x3 + halfxscale, y2 - (short) ((taArrayItem(b1.hi, pos) - ystart) * yscale));
   lineto(x3 + halfxscale, y2 - (short) ((taArrayItem(b1.lo, pos) - ystart) * yscale));
   if (!taArrayItem(b1.op, pos) || taArrayItem(b1.op, pos) == taArrayItem(b1.cl, pos))
   {
    moveto(x3, y2 - (short) ((taArrayItem(b1.cl, pos) - ystart) * yscale));
    lineto(x3 + xscale - i1, y2 - (short) ((taArrayItem(b1.cl, pos) - ystart) * yscale));
    continue;
   }
   if (taArrayItem(b1.op, pos) > taArrayItem(b1.cl, pos)) /* down */
    setcolor((short)style);
   else                                                   /*  up  */
    setcolor((short)color2);
   rectangle(x3 + 1, y2 - (short) ((taArrayItem(b1.cl, pos) - ystart) * yscale),
	  x3 - 1 + xscale - i1, y2 - (short) ((taArrayItem(b1.op, pos) - ystart) * yscale));
   setcolor((short)color1);
   rectangle(x3, y2 - (short) ((taArrayItem(b1.cl, pos) - ystart) * yscale),
	  x3 + xscale - i1, y2 - (short) ((taArrayItem(b1.op, pos) - ystart) * yscale));
  }
  break;
 ...
 case taCHART_HIST:
  ...
  {
   x3 = (pos - xstart) * xscale + 1;
   rectangle(x3, y3, x3 + xscale - 2, y2 - (short) ((taArrayItem(a1, pos) - ystart) * yscale));
   if (style != taSOLID)
    rectangle(x3, y3, x3 + xscale - 2, y2 - (short) ((taArrayItem(a1, pos) - ystart) * yscale));
  }
  //setfillmask(NULL);
 ...
 case taCHART_LINE:
  ...
  moveto((pos - xstart - 1) * xscale + 1, y2 - (short) ((taArrayItem(a1, pos - 1) - ystart) * yscale));
  for (; pos <= xstop && pos < a1.size; pos++)
  {
   x3 = (pos - xstart) * xscale + halfxscale;
   lineto(x3, y2 - (short) ((taArrayItem(a1, pos) - ystart) * yscale));
  }
  setlinestyle(taSOLID,0,0);
  break;
 ...
 //setviewport(0, 0, config.numxpixels - 1, config.numypixels - 1);
 return (err);
}


int    taChartGrid(double level, double ystart, double ystop,
...
int DLL_EXPORT   taChartScreenInit(struct taChartScreen *screen, int linemax, int gridmax)
{
 ...
 screen->chartline = malloc(linemax * sizeof(struct taChartLine));
 if (screen->chartline == NULL)
  return (taErrMalloc);
 screen->chartgrid = malloc(gridmax * sizeof(struct taChartGrids));
...
void     taChartScreenClose(struct taChartScreen *screen)
{
 free(screen->chartline);
...
int    taChartSetup(struct taChartScreen *screen, struct taChartInfo *info)
{
 int    i1, i2, i3;
 ...
 setcolor((short)screen->bgdcolor);
 rectangle(screen->x1, screen->y1, screen->x2, screen->y2);
 setcolor((short)screen->linecolor);
 ...
 if (screen->heading[0])
 {
  ...
  setcolor((short)screen->fgdcolor);
...
int    taChartPaint(struct taChartScreen *screen, struct taChartInfo *info)
{
 int    i1, i2, i3, i4;
 int    yr, mo, da;
 double    f1, f2;
 ...
 if (screen->datestyle && !taIsNoBars(info->masterbars) && (taArrayItem(info->masterbars.dt, screen->xstop) - taArrayItem(info->masterbars.dt, screen->xstart)) >= 1)
 {
  ...
  for (i1 = 0; i1 < screen->xwidth && screen->xstart + i1 < screen->xstop; i1++)
  {
   ...
   if (buffer[0])
   {
    ...
    setcolor((short)screen->numcolor);
    ...
    {
     setcolor((short)screen->gridcolor);
...
int    taChartDraw(struct taChartScreen *screen)
{
...
int    taChartPosition(struct taChartScreen *screen, struct taChartInfo *info,
