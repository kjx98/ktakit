...
KFloat DLL_EXPORT taApprxCumProbDist(KFloat d1)
{
    ...
    if (d1 > 0) acpd = 1 - d4 * d3 * d5;
    else acpd = 1 - (1 - d4 * d3 * d5);
    return (acpd);
}
...
KFloat DLL_EXPORT taBondDuration(struct taBondInfo *bond, KFloat discrate, unsigned int asofdate)
{
    ...
    for (i1 = 0; i1 < yrs; i1++)
    {
        if (i1 < yrs - 1) f1 = bond->coupon * (i1 + 1);
        else f1 = (bond->coupon + bond->facevalue) * (i1 + 1);
        tvalue.principal = f1;
...
{
    if (discrate < 1) return (taErrInvalidParm);
...
KFloat DLL_EXPORT taBondPricing(struct taBondInfo *bond, KFloat discrate, unsigned int asofdate)
{
    ...
    {
        if (f0 < yrs) f1 = c1 / pow(1 + ((discrate / 100)), f0);
        else f1 = (c1 + bond->facevalue) / pow(1 + ((discrate / 100)), f0);
        f2 += f1;
    }
    return (f2);
}
...
KFloat DLL_EXPORT taFutureValue(struct taTimeValue *tvalue)
{
    ...
    if (tvalue->compound > 0) f1 = f1 / tvalue->compound;
    else return (taErrInvalidParm);
...
KFloat DLL_EXPORT taPresentValueAnnuity(struct taTimeValueA *tvaluea)
{
    ...
    if (tvaluea->discrate > 0) f1 = tvaluea->discrate / 100;
    else return (taErrInvalidParm);
